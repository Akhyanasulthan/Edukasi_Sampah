...
@section('page-title', 'Edit Soal Evaluasi')
@section('breadcrumb', 'Beranda / Kelola Soal / Edit')
...
<div class="card">
    <form action="{{ route('admin.evaluation-questions.update', $evaluationQuestion->id) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div class="form-group">
            <label class="form-label">Pertanyaan</label>
            <input type="text" name="question" class="form-control" required value="{{ old('question', $evaluationQuestion->question) }}">
        </div>

        <div class="form-group">
            <label class="form-label">Tipe Soal</label>
            <select name="type" id="typeSelect" class="form-control" required>
                <option value="text" {{ $evaluationQuestion->type == 'text' ? 'selected' : '' }}>Teks Singkat</option>
                <option value="number" {{ $evaluationQuestion->type == 'number' ? 'selected' : '' }}>Angka</option>
                <option value="radio" {{ $evaluationQuestion->type == 'radio' ? 'selected' : '' }}>Pilihan Ganda (Radio)</option>
                <option value="textarea" {{ $evaluationQuestion->type == 'textarea' ? 'selected' : '' }}>Teks Panjang (Paragraf)</option>
            </select>
        </div>

        <div class="form-group" id="optionsGroup" style="display: {{ $evaluationQuestion->type == 'radio' ? 'block' : 'none' }};">
            <label class="form-label">Pilihan Jawaban (Pisahkan dengan koma)</label>
            <input type="text" name="options" class="form-control" placeholder="Contoh: Sangat Baik, Baik, Cukup, Kurang" value="{{ old('options', $evaluationQuestion->options ? implode(', ', $evaluationQuestion->options) : '') }}">
        ...
        <div class="form-group">
            ...
            <input type="number" name="order_column" class="form-control" value="{{ old('order_column', $evaluationQuestion->order_column) }}" required>
        </div>

        <div class="form-group" style="display: flex; align-items: center; gap: 8px;">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ $evaluationQuestion->is_active ? 'checked' : '' }}>
